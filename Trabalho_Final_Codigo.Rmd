...
### 4° Modelo -  TESTE NEGATIVE BINOMIAL
...
```
### 5° Modelo - Ajustando um Modelo de Regressão de Poisson Quase (QPR):
```{r fig.align='center', fig.height=8, fig.width=8, message=FALSE, warning=FALSE}

# Ajustando um modelo de Poisson Quase (QPR)
modelo <- glm(FatScore ~ BodyMass + Heterophils + Eosinophils + Monocytes + Basophils + Genero, data = df, family = quasipoisson())
...
df$FatScore <- ifelse(df$FatScore == 0, 1, df$FatScore)
